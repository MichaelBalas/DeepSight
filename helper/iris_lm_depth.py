...
from helper.core import (detections_to_rect, landmarks_to_detections, slice_from_roi, tflite_inference, transform_rect)


def from_landmarks_to_depth(frame_rgb, eye_landmarks, image_size, is_right_eye=False, focal_length=None):
...
def calculate_depth(center_landmark, focal_length_pixel, iris_size, image_size):
    ...
    human_iris_size_in_mm = 11.7
    #human_iris_size_in_mm = 9.04
    origin = np.array(image_size) / 2.0
    center_landmark_pixel = center_landmark[:2] * np.array(image_size)
    y = get_depth(origin[0], origin[1], center_landmark_pixel[0], center_landmark_pixel[1])
